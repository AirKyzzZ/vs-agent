...
describe('TrustClient', () => {
  it('resolve maps 200 to ok with trustStatus', async () => {
    const client = new TrustClient(
      RESOLVER,
      fetchReturning(200, { did: DID, trustStatus: 'TRUSTED', production: true }),
    )
  ...
  it('resolve maps 404 to not_found and 500 to unreachable', async () => {
    ...
    expect(await new TrustClient(RESOLVER, fetchReturning(500)).resolve(DID)).toEqual({
      status: 'unreachable',
    })
  })
  ...
  it('checkAuthorization returns boolean on 200, false on 404, null otherwise', async () => {
    expect(
      await new TrustClient(RESOLVER, fetchReturning(200, { authorized: true })).checkAuthorization(
        'verifier',
        DID,
        'https://jsc',
      ),
    ...
    expect(
      await new TrustClient(RESOLVER, fetchReturning(404)).checkAuthorization('verifier', DID, 'https://jsc'),
    ).toBe(false)
    expect(
      await new TrustClient(RESOLVER, fetchReturning(503)).checkAuthorization('verifier', DID, 'https://jsc'),
    ).toBe(null)
  })
  it('verdictFor with null did is UNTRUSTED with note', async () => {
    const { verdict, evidence } = await new TrustClient(RESOLVER, fetchReturning(200, {})).verdictFor(
      'verifier',
      null,
      'https://jsc',
    )
  ...
  it('verdictFor with null vtjscId never authorizes', async () => {
    const { verdict } = await new TrustClient(
      RESOLVER,
      fetchReturning(200, { trustStatus: 'TRUSTED' }),
    ).verdictFor('verifier', DID, null)
    expect(verdict).toBe('TRUSTED_NOT_AUTHORIZED')
  })
  it('verdictFor records the exact query urls in evidence', async () => {
    const { evidence } = await new TrustClient(
      RESOLVER,
      fetchReturning(200, { trustStatus: 'TRUSTED', authorized: true }),
    ).verdictFor('verifier', DID, 'https://jsc')
    expect(evidence.queries[0]).toBe(`${RESOLVER}/resolve?did=${encodeURIComponent(DID)}`)
    expect(evidence.queries[1]).toBe(
      `${RESOLVER}/verifier-authorization?did=${encodeURIComponent(DID)}&vtjscId=${encodeURIComponent('https://jsc')}`,
    )
  })
  it('treats malformed JSON bodies as unreachable/unknown', async () => {
    ...
    expect(await new TrustClient(RESOLVER, badJson).checkAuthorization('verifier', DID, 'https://jsc')).toBe(
      null,
    )
  })
  it('never authorizes on non-boolean authorized values', async () => {
    ...
    expect(
      await new TrustClient(RESOLVER, fetchReturning(200, { authorized: 1 })).checkAuthorization(
        'verifier',
        DID,
        'https://jsc',
      ),
  ...
  it('treats off-spec trustStatus as untrusted in the verdict', async () => {
    const { verdict } = await new TrustClient(
      RESOLVER,
      fetchReturning(200, { trustStatus: 'FOO', authorized: true }),
    ).verdictFor('verifier', DID, 'https://jsc')
    expect(verdict).toBe('UNTRUSTED')
  })
})
